...
export function JobsTable({ postings }: { postings: Posting[] }) {
  return (
    <div id="jobs" className="border-[1.5px] border-stone-200 rounded-[10px] bg-white overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-stone-100">
        <div className="flex items-center gap-1.5">
          <span className="text-orange-600 font-mono text-xs">▸</span>
          <span className="font-mono text-[13px] font-bold text-stone-950">recent_postings</span>
          <span className="text-[10px] bg-orange-50 text-orange-600 px-1.5 rounded font-semibold border border-orange-200">new</span>
        </div>
      ...
      {postings.length === 0 ? (
        ...
        postings.map((job) => (
          <div key={job.id} className="grid grid-cols-[2fr_1.2fr_1fr_0.6fr] gap-1.5 px-5 py-2.5 border-b border-stone-50 items-center">
            <span className="text-xs font-semibold text-stone-950 truncate">{job.title}</span>
            <div className="flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: CATEGORY_DOT_COLORS[job.category] ?? "#a8a29e" }} />
              <span className="text-xs text-stone-700 truncate">{job.company}</span>
            </div>
            <span className="text-[11px] text-stone-500 truncate hidden md:block">{job.location ?? "—"}</span>
